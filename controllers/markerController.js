...
async function removeMarker(req, res, next) {
  ...
  await UserMapapp.findOne({ _id: req.user._id }, (err, user) => {
    ...
    user.markers.splice(
      user.markers.findIndex((e) => markerId === e._id.toString()),
      1
    );
    user.save((err) => {
      if (err) return res.json(err);
      else console.log("Task has been deleted");
    });
    return res.json("Task deleted successfull");
  });
}
